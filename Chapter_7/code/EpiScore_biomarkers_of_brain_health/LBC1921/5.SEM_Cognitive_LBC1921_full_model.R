...
#set sex as factor and scale age 
data$sex <- as.factor(data$sex)
data$age <- as.vector(scale(dset_mod$age))

#read in epismoker and rename id 
epismoke <- readRDS("lbc_epismoker.rds") 
names(epismoke)[1] <- "Basename"

#combine phenotype and epismoker
data <- data %>% left_join(epismoke, by = "Basename")

#scale covariates 
data$alcpw <- as.vector(scale(data$alcpw))
data$bmi <- as.vector(scale(data$bmi))
data$soclcode <- as.vector(scale(data$soclcode))
data$smokingScore <- as.vector(scale(data$smokingScore))

#rename dataset for use in script 
dset_mod <- data
...
#create measurement model of general cognitive function and change
general_4p <- '
...
#saves out results for each episcore: G intercept ~ episcore (tmp) + age + sex + BMI + occupational social class + alcohol units + epismoker 
foreach(i = episcores, .packages = c("lavaan")) %dopar% { 
  
  dset_mod$tmp = dset_mod[,i]
  
  reg_Ig <- '
 Ig ~ tmp + age + sex + alcpw + bmi + smokingScore + soclcode 
'
  ...
  write.csv(results, paste0(i, "_G_intercept_No_Domain_LBC1921_assocs_84_EpiScores_full_model_no_yrsedu_21042023.csv"), row.names = F)
...
#for loop that loops over episcores using parallel processing to perform regression in SEM
#saves out results for each episcore: G slope ~ episcore (tmp) + age + sex + BMI + occupational social class + alcohol units + epismoker 
foreach(i = episcores, .packages = c("lavaan")) %dopar% { 
  
  dset_mod$tmp = dset_mod[,i]
  
  reg_Sg <- '
 Sg ~ tmp + age + sex + alcpw + bmi + smokingScore + soclcode  
'
  ...
  write.csv(results, paste0(i, "_G_slope_No_Domain_LBC1921_assocs_84_EpiScores_full_model_no_yrsedu_21042023.csv"), row.names = F)
