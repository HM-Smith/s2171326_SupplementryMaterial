...
#read in and combine with epismoker scores
epismoke <- readRDS("lbc_epismoker.rds") 
names(epismoke)[1] <- "Basename"
LBC <- LBC %>% left_join(epismoke, by = "Basename")

#missing code = 99999
summary(LBC$depind_w1)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max.
#4    3134    5354    5265    6264   99999

#recode missing code to NA
LBC$depind_w1 <- ifelse(LBC$depind_w1 == 99999, NA, LBC$depind_w1)
...
#scale age and Intracranial volume 
...
#scale covariates 
LBC$depind_w1 <- as.vector(scale(LBC$depind_w1))
LBC$bmi_w1 <- as.vector(scale(LBC$bmi_w1))
LBC$smokingScore <- as.vector(scale(LBC$smokingScore))
LBC$alcunitwk_w1 <- as.vector(scale(LBC$alcunitwk_w1))


#rescale the MRI variables to aid in model convergence 
...
#create two empty results dataframe 
...
for(i in episcores) { 
  ...
  GM_model_reg <-  '
    ...
    gm_mm3_w5 ~ Age_w5 
    tmp ~ Age_w1
    IGMV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 + ICV_mm3_wX
    SGMV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 
...
write.csv(results, file = "GreyMatter_intercept_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
write.csv(results_slope, file ="GreyMatter_slope_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
...
#for loop for looping over episcores and performs regression analysis 
for(i in episcores) { 
  LBC$tmp = LBC[,i]
  ...
  Brain_model_reg <-  '
   ...
   # regressions 
     ...
     brain_mm3_w5 ~ Age_w5 
     tmp ~ Age_w1
     ITMV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 + ICV_mm3_wX
     STMV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 
    '
...
#save out results files 
write.csv(results, file = "Totalbrain_intercept_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
write.csv(results_slope, file ="Totalbrain_slope_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
...
#for loop for looping over episcores and performing regression analysis 
for(i in episcores) { 
  LBC$tmp = LBC[,i]
  ...
  nawm_mm3_w5 ~ Age_w5 
  tmp ~ Age_w1
  INAWMV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 + ICV_mm3_wX
  SNAWMV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 
  '
...
#save out results files
write.csv(results, file = "NAWM_intercept_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
write.csv(results_slope, file ="NAWM_slope_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
...
for(i in episcores) { 
  ...
  # latent variables
   ...
   # regressions 
     ...
     wmh_mm3_log_w5 ~ Age_w5
     tmp ~ Age_w1
     IWMHV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 + ICV_mm3_wX
     SWMHV ~ tmp + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1
  '
...
#save out results 
write.csv(results, file = "WMH_intercept_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
write.csv(results_slope, file ="WMH_slope_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv")
