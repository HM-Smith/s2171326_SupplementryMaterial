...
#rescale cognitive tests to aid in model convergence 
...
dset_mod$bmi_w1 <- scale(dset_mod$bmi_w1)
...
#create measurement model of general cognitive function and change - intercept and slope, respectively
general_4p <- '
...
list_e <- colnames(dset_mod[335:418])
episcores = colnames(dset_mod)[which(colnames(dset_mod)%in% list_e)]

#for loop for looping over episcores and performing regression analysis with intercept as outcome and saving out results
foreach(i = episcores, .packages = c("lavaan")) %dopar% { 
  
  dset_mod$tmp = dset_mod[,i]
  
  reg_Ig <- '
 Ig ~ tmp + Age_w1 + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 
'
  ...
  write.csv(results, paste0(i, "_G_intercept_No_Domain_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv"), row.names = F)
  
  
}

#for loop for looping over episcores and performing regression analysis with slope as outcome and saving out results
foreach(i = episcores, .packages = c("lavaan")) %dopar% { 
  
  dset_mod$tmp = dset_mod[,i]
  
  reg_Sg <- '
 Sg ~ tmp + Age_w1 + sex + bmi_w1 + smokingScore + depind_w1 + alcunitwk_w1 
'
  ...
  write.csv(results, paste0(i, "_G_slope_No_Domain_LBC1936_assocs_84_EpiScores_fullmodel_no_yrsedu_27102023.csv"), row.names = F)
