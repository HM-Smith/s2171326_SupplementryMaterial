...
#remove harmony and parkinsons study
pheno_combined <- pheno_combined[!(pheno_combined$STUDY %in% c("Harmony", "Parkinson")), ]

#read in nfl, rename columns and select columns of interest
response = read.csv("NFL.csv")
...
#remove studies that dont' have methylation and proteins 
...
results_list <- foreach(j = CpGs, .packages = c("lmerTest", "lme4"), .combine = 'rbind') %dopar% {
  ...
  mod <- lmer(scale(log_NFL) ~ scale(meth_data[,j]) + 
                scale(AGE) + 
                as.factor(SEX) +
                (1|PAIRID) +
                (1|TWINNR) +
...
#save out file
write.csv(results_list, file = "NFL_EWAS_TWINNR.csv")
