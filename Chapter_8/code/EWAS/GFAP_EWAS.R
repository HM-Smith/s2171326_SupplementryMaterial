...
#remove harmony and parkinson studies
pheno_combined <- pheno_combined[!(pheno_combined$STUDY %in% c("Harmony", "Parkinson")), ]

#read in GFAP, rename columns to match other files and select columns of interest
response = read.csv("GFAP.csv")
...
#remove 7th time points where there isn't methylation and proteins 
...
results_list <- foreach(j = CpGs, .packages = c("lmerTest", "lme4"), .combine = 'rbind') %dopar% {
  ...
  mod <- lmer(scale(log_GFAP) ~ scale(meth_data[,j]) + 
                scale(AGE) + 
                as.factor(SEX) +
                (1|PAIRID) +
                (1|TWINNR) +
...
#save out results 
write.csv(results_list, file = "GFAP_EWAS_TWINNR.csv")
