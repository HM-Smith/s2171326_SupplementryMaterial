...
loop=loop[grep("_processed", loop)]
library(data.table) 
for(i in loop){
  # Extract name 
  nm = gsub(".csv", "", i)
  # Read in CpG list in order 
  names=read.csv("../DNAm_cpg_list.csv",header=F)
...
L <- list.files(".", "_processed_pip.csv")

pip_files <- lapply(L, read.csv, header = T) 
names <- as.character(L) 
names <- gsub("_processed_pip.*", "", names) 
...
write.csv(tmp1, paste0("Group_PIP/", i, "_", names(pip_files)[[j]],".csv"),row.names=F)
}
# Print to denote completion 
