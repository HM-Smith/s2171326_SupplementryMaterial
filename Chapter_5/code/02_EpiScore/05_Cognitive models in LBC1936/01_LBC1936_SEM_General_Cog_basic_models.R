...
#select appropriate columns e.g, cog tests, age, sex 
cog <- select(cog, -bmi_w1, -sex)

#select episcores and measured traits
...
#log transform measured bmi
data$bmi_w1 <- log(data$bmi_w1)

#scale episcores and measured traits 
...
#growth curve model for general function 
...
#detect and register cores for parallel processing
cores <- detectCores()
cl <- makeCluster(9)
registerDoParallel(cl)

#make list of episcores and measured traits
...
'
  ...
  #extracts beta, se, p-value, CI's, fit measures 
  ...
  #create empty results dataframe to write out results to 
  ...
  #save out results 
  write.csv(results, paste0(i, "_G_intercept_No_Domain_LBC1936_assocs_lipid_EpiScores_basic.csv"), row.names = F)
...
'
  ...
  #extract beta, se, p-value, CI's, fit measures 
  output = standardizedSolution(fitreg_g)
  ...
  #make empty empty dataframe for results to be written out too 
  results <- data.frame(SeqId = NA, n = NA, beta = NA , SE = NA, P = NA, ci.upper = NA, ci.lower = NA, cfi = NA, rmsea = NA, srmr = NA, tli = NA)
  ...
  #save out results 
  write.csv(results, paste0(i, "_G_slope_No_Domain_LBC1936_assocs_lipid_EpiScores_basic.csv"), row.names = F)
